...
<div
  class="w-full grid xl:grid-cols-2 xl:grid-rows-1 grid-rows-5 lg:gap-10 bg-white shadow-sm p-5 rounded-md gap-5"
>
  <div
    class="rounded-md bg-gray-300 bg-cover bg-center shadow-sm relative hover:cursor-pointer row-span-2 xl:row-span-3"
  ...
  <div class="flex flex-col justify-between gap-7 row-span-3" class:xl:row-start-1={inverted}>
    <div class="flex flex-col gap-2">
      ...
      <h2 class="text-lg text-justify font-normal text-gray-600 whitespace-normal">
        {content.replace(/(<([^>]+)>)/gi, "").slice(0, 300) + "..."}
